  ...
  return (
    <>
      ...
      <button
        ...
        className="fixed top-4 left-4 z-20 sm:hidden text-white bg-gray-800 p-2 rounded-md shadow"
      >
      ...
      <aside
        className={`
          fixed top-0 left-0 h-full bg-[#111827] text-gray-200 p-5 flex flex-col justify-between border-r border-gray-700 shadow-lg
      ...
      >
        <div>
          ...
          <nav className="space-y-2 text-sm">
            {[
              '서든어택','오버워치','배틀그라운드','발로란트',
              '리그오브레전드','에이펙스','하드스푸퍼','팀뷰기'
            ].map((game, idx) => (
              <button
                key={idx}
